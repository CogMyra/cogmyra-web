...
export async function onRequestPost({ request, env }) {
  ...
  // Normalize -> rows (default user_id to 'anon')
  const rows = [];
  for (const ev of list) {
    const type = String(ev?.type || "").trim();
    if (!type) continue;

    const userId = String(ev?.user_id || "anon").slice(0, 128);
  ...
  // Ensure referenced users exist (satisfy FK)
  const distinctUsers = [...new Set(rows.map(r => r.user_id))];
  try {
    const upsertStmt = env.cmg_db.prepare(
      "INSERT OR IGNORE INTO users (id, created_at) VALUES (?, datetime('now'))"
    );
    await env.cmg_db.batch(distinctUsers.map(u => upsertStmt.bind(u)));
  } catch (err) {
    // If this fails, still try to insert events; we'll surface any FK error then
  }
  ...
  `;
  try {
    const stmt = env.cmg_db.prepare(insertSql);
